...
class ControlGUI:
    ...
    def create_gui(self, thread_list):
        ...
        """
        layout = [
            [sg.T("Rip existing threads or add new ones?")],
            [sg.Button("Rip existing")],
            [sg.Button("Add new")],
            [sg.Button("Show current threads")],
        ]
        window = sg.Window(
            "Mongolian Basket Weaving Thread Archiver",
            layout,
...
class GUIRipThread:
    ...
    def save_thread(self):
        ...
        layout = [
            ...
            [
                sg.T(
                    "Any archived or 404ed threads will be deleted from your"
                    + " saved file"
                )
            ],
        ]
        window = sg.Window(
            "Unofficial Mongolian Basket Weaving Thread" + " Archiver",
            layout,
            enable_close_attempted_event=True,
        ...
        while True:
            ...
            if event in ("Start rip", sg.TIMEOUT_KEY):
                    ...
                    try:
                        ...
                        if is_archived == 2:
                            ...
                            window.perform_long_operation(
                                lambda: self.post_ripper(thread, complete_folder_path),
                                "-FUNCTION COMPLETED-",
                            )
                            try:
                                new_loop_thread_list.remove(thread)
                ...
                window["-OUTPUT_TEXT-"].update(
                    "Rip complete!" + "The script will re-start in 60 seconds."
                )
    ...
    def convert_to_json(self):
        ...
        """
        self.json_formatted = self.thread.replace("boards.4channel.org", "a.4cdn.org")
        self.json_formatted = self.json_formatted.replace(
            "boards.4chan.org", "a.4cdn.org"
        )
        self.json_formatted = self.json_formatted.replace("[", "")
        self.json_formatted = self.json_formatted.replace("]", "")
        self.json_formatted = self.json_formatted.replace("'", "")
    ...
    def api_call(self):
        ...
        for item in api_json["posts"]:
            ...
            if "tim" in item:
                ...
                completed_image_url = (
                    "https://i.4cdn.org/" + board_name[3] + "/" + filename + extension
                )
        ...
        for picture in self.image_url_list:
            complete_folder_path = os.path.join(
                board_name[3], board_name[5].replace(".json", "")
            )
...
class GUIGetThread:
    ...
    def get_user_input(self):
        ...
        layout = [
            [
                sg.T(
                    "Please enter your list of threads,"
